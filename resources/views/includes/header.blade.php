...
<body class="text-slate-800 flex h-screen overflow-hidden bg-[#f4f7fe]">
  ...
  <aside id="sidebar" class="fixed inset-y-0 left-0 w-[260px] bg-white flex flex-col shadow-2xl lg:shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-50 border-r border-slate-100 transform -translate-x-full transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-auto h-full flex-shrink-0">
      ...
      <div class="p-5">
          <div class="bg-red-live-gradient rounded-3xl p-5 text-center text-white shadow-lg shadow-red-200/60 relative overflow-hidden">
              ...
              <div class="w-8 h-8 bg-white/20 rounded-xl flex items-center justify-center mx-auto mb-2 backdrop-blur-sm"><i class="fas fa-file-pdf text-sm"></i></div>
  ...
  <div class="flex-1 flex flex-col h-screen overflow-hidden relative w-full">
      <header class="h-16 sm:h-20 bg-[#f4f7fe]/90 backdrop-blur-xl flex items-center justify-between px-4 sm:px-8 z-30 sticky top-0 border-b border-transparent">
          ...
          <div class="flex items-center gap-2 sm:gap-4">
              ...
              <div class="relative">
                  <button onclick="toggleNotifDropdown()" id="notif-bell-btn" class="relative w-8 h-8 sm:w-10 sm:h-10 bg-white rounded-full flex items-center justify-center shadow-sm transition btn-animated {{ $pendingCount > 0 ? 'text-red-500 hover:text-red-700' : 'text-slate-500 hover:text-red-600' }}" title="Notifications">
                      <i id="notif-bell-icon" class="far fa-bell {{ $pendingCount > 0 ? 'animate-ring' : '' }}"></i>
                      <span id="notif-badge-container" class="absolute top-0 right-0 sm:top-0.5 sm:right-0.5 flex h-3 w-3 sm:h-4 sm:w-4 items-center justify-center {{ $pendingCount > 0 ? '' : 'hidden' }}">
                          <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
                          <span id="notif-badge-count" class="relative inline-flex rounded-full h-3 w-3 sm:h-4 sm:w-4 bg-red-600 text-white text-[8px] sm:text-[9px] font-bold items-center justify-center border-2 border-white">{{ $pendingCount > 9 ? '9+' : $pendingCount }}</span>
                      </span>
                  </button>
                  <div id="notif-dropdown" class="hidden absolute right-0 mt-3 w-64 sm:w-72 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50 animate-slide-up">
                      <div class="bg-slate-50 px-4 py-3 border-b border-slate-100 flex justify-between items-center">
                          ...
                          <span id="notif-dd-count" class="bg-red-100 text-red-600 text-[9px] font-black px-2 py-0.5 rounded-full {{ $pendingCount > 0 ? '' : 'hidden' }}">{{ $pendingCount }} New</span>
                      </div>
                      <div id="notif-dd-body" class="max-h-64 overflow-y-auto custom-scroll">
                          @if($pendingCount > 0)
                              <div onclick="goToApproval('{{ $targetFilter }}')" class="p-4 border-b border-slate-50 hover:bg-red-50 cursor-pointer transition flex gap-3 group">
              ...
              @if($isAdmin)
              <button onclick="openSettingsModal()" class="w-8 h-8 sm:w-10 sm:h-10 bg-white rounded-full flex items-center justify-center text-slate-500 hover:text-red-600 shadow-sm transition btn-animated" title="System Settings"><i class="fas fa-cog text-red-500 animate-[spin_4s_linear_infinite]"></i></button>
              <button onclick="openManageUsers()" class="w-8 h-8 sm:w-10 sm:h-10 bg-white rounded-full flex items-center justify-center text-slate-500 hover:text-red-600 shadow-sm transition btn-animated" title="Manage Users"><i class="fas fa-users-cog"></i></button>
              @endif
      ...
      <div id="modal-prompt-custom" class="hidden fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div class="bg-white rounded-3xl w-full max-w-sm shadow-2xl animate-slide-up overflow-hidden"><div class="p-6 sm:p-8 text-center"><div class="w-14 h-14 bg-indigo-100 rounded-full flex items-center justify-center mx-auto mb-4 text-indigo-600 shadow-inner"><i class="fas fa-edit text-xl"></i></div><h3 class="text-lg font-black text-slate-800 mb-2 tracking-tight" id="prompt-custom-title" data-translate="true">Edit Data</h3><p class="text-xs sm:text-sm text-slate-500 mb-4 leading-relaxed font-medium" id="prompt-custom-msg" data-translate="true">Masukkan nilai baru:</p><input type="text" id="prompt-custom-input" class="w-full border border-slate-300 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition font-bold text-center mb-6" placeholder="Ketik disini..."><div class="flex gap-2"><button onclick="closeModal('modal-prompt-custom')" class="flex-1 py-3 border-2 border-slate-200 text-slate-600 rounded-xl font-bold text-xs hover:bg-slate-50 transition btn-animated" data-translate="true" data-i18n="btn_cancel">Batal</button><button onclick="executeCustomPrompt()" class="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold text-xs hover:bg-indigo-700 shadow-md transition btn-animated" data-translate="true">Simpan</button></div></div></div>
      </div>
                ...
                <div class="w-full sm:w-1/3 border-b sm:border-b-0 sm:border-r border-slate-200 flex flex-col bg-slate-50 h-1/3 sm:h-full">
                    <div class="p-4 border-b border-slate-200">
                        <div class="grid grid-cols-4 gap-2 mb-3">
                            <button onclick="resetUserForm()" class="col-span-1 bg-red-600 text-white py-2 rounded-xl font-bold text-xs shadow-md hover:bg-red-700 btn-animated flex justify-center items-center" title="New User Manual"><i class="fas fa-user-plus"></i></button>
                            <button onclick="downloadUserTemplate()" class="col-span-1 bg-blue-500 text-white py-2 rounded-xl font-bold text-xs shadow-md hover:bg-blue-600 btn-animated flex justify-center items-center" title="Download Template Excel"><i class="fas fa-file-excel"></i></button>
                            <button onclick="document.getElementById('import-user-file').click()" class="col-span-1 bg-emerald-500 text-white py-2 rounded-xl font-bold text-xs shadow-md hover:bg-emerald-600 btn-animated flex justify-center items-center" title="Upload Data User (Excel)"><i class="fas fa-upload"></i></button>
                            <button onclick="exportUsers()" class="col-span-1 bg-amber-500 text-white py-2 rounded-xl font-bold text-xs shadow-md hover:bg-amber-600 btn-animated flex justify-center items-center" title="Export Semua User"><i class="fas fa-download"></i></button>
                        </div>
                        <input type="file" id="import-user-file" accept=".xlsx, .xls, .csv" class="hidden" onchange="importUsers(event)">
                        <div class="relative"><span class="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-400"><i class="fas fa-search text-xs"></i></span><input type="text" id="search-user" onkeyup="filterUsers()" class="w-full border border-slate-300 rounded-xl p-2 pl-8 text-xs outline-none focus:ring-2 focus:ring-red-500 shadow-sm transition" data-translate-ph="true" placeholder="Search user..."></div>
                    </div>
  ...
  <script>
    ...
    let userRights = [];
    if (typeof rawRights === 'string') { try { userRights = JSON.parse(rawRights); } catch(e) { userRights = []; } }
    else if (Array.isArray(rawRights)) { userRights = rawRights; }

    let inventoryData = []; let globalConfirmCallback = null; let allUsers = [];
    let currentLang = localStorage.getItem('portal_lang') || 'en';
    let globalPromptCallback = null;

    const IDLE_TIMEOUT_MINUTES = 2; let idleTime = 0; let idleInterval;
    ...
    function showCustomPrompt(title, message, defaultValue, callback) { document.getElementById('prompt-custom-title').innerText = title; document.getElementById('prompt-custom-msg').innerText = message; document.getElementById('prompt-custom-input').value = defaultValue || ''; globalPromptCallback = callback; openModal('modal-prompt-custom'); setTimeout(() => document.getElementById('prompt-custom-input').focus(), 100); if(typeof AutoTranslator !== 'undefined') AutoTranslator.processDOM(); }
    function executeCustomPrompt() { const val = document.getElementById('prompt-custom-input').value; if(globalPromptCallback) globalPromptCallback(val); closeModal('modal-prompt-custom'); globalPromptCallback = null; }

    function toggleSidebar() { const sidebar = document.getElementById('sidebar'); const overlay = document.getElementById('sidebar-overlay'); if (sidebar.classList.contains('-translate-x-full')) { sidebar.classList.remove('-translate-x-full'); overlay.classList.remove('hidden'); document.body.classList.add('overflow-hidden'); setTimeout(() => overlay.classList.remove('opacity-0'), 10); } else { sidebar.classList.add('-translate-x-full'); overlay.classList.add('opacity-0'); document.body.classList.remove('overflow-hidden'); setTimeout(() => overlay.classList.add('hidden'), 300); } }
    ...
    const AutoTranslator = {
        ...
        async translate(text, targetLang) {
            if (!text || !text.trim()) return text; const key = text.trim() + "_" + targetLang; if (this.cache[key]) return this.cache[key];
        ...
        async processDOM() {
            ...
            for(let el of elements) { let originalText = el.getAttribute('data-orig-text'); if(!originalText) { originalText = el.innerText.trim(); el.setAttribute('data-orig-text', originalText); } if(!originalText) continue; const i18nKey = el.getAttribute('data-i18n'); if(i18nKey && i18n[currentLang] && i18n[currentLang][i18nKey]) { el.innerHTML = i18n[currentLang][i18nKey]; continue; } const translated = await this.translate(originalText, currentLang); if(el.innerText !== translated) el.innerText = translated; }
            const inputs = document.querySelectorAll('[data-translate-ph="true"]');
            for(let el of inputs) { let originalPh = el.getAttribute('data-orig-ph'); if(!originalPh) { originalPh = el.placeholder.trim(); el.setAttribute('data-orig-ph', originalPh); } if(!originalPh) continue; const i18nKey = el.getAttribute('data-i18n-ph'); if(i18nKey && i18n[currentLang] && i18n[currentLang][i18nKey]) { el.placeholder = i18n[currentLang][i18nKey]; continue; } const translated = await this.translate(originalPh, currentLang); if(el.placeholder !== translated) el.placeholder = translated; }
        }
    };

    async function applyLanguage() { document.getElementById('lang-label').innerText = currentLang.toUpperCase(); document.querySelectorAll('[data-i18n]:not([data-translate="true"])').forEach(el => { const k = el.getAttribute('data-i18n'); if(i18n[currentLang] && i18n[currentLang][k]) el.innerHTML = i18n[currentLang][k]; }); document.querySelectorAll('[data-i18n-ph]:not([data-translate-ph="true"])').forEach(el => { const k = el.getAttribute('data-i18n-ph'); if(i18n[currentLang] && i18n[currentLang][k]) el.placeholder = i18n[currentLang][k]; }); await AutoTranslator.processDOM(); }
    ...
    function logoutAction() { fetch('{{ url("api/auth.php") }}', { method:'POST', body:JSON.stringify({action:'logout'}) }).then(() => { window.location.href = '{{ url("/login") }}'; }).catch(() => { window.location.href = '{{ url("/login") }}'; }); }
    ...
    function showCustomAlert(title, message) { document.getElementById('alert-custom-title').innerText = title; document.getElementById('alert-custom-msg').innerText = message; document.getElementById('alert-custom-title').setAttribute('data-orig-text', title); document.getElementById('alert-custom-msg').setAttribute('data-orig-text', message); openModal('modal-alert-custom'); if(typeof AutoTranslator !== 'undefined') AutoTranslator.processDOM(); }
    function showCustomConfirm(title, message, callback) { document.getElementById('confirm-custom-title').innerText = title; document.getElementById('confirm-custom-title').setAttribute('data-orig-text', title); document.getElementById('confirm-custom-msg').innerText = message; document.getElementById('confirm-custom-msg').setAttribute('data-orig-text', message); globalConfirmCallback = callback; openModal('modal-confirm-custom'); if(typeof AutoTranslator !== 'undefined') AutoTranslator.processDOM(); }
    ...
    function renderDropdownItems(inp, type) {
        ...
        if(filtered.length === 0) { list.innerHTML = `<div class="p-2 text-[10px] text-slate-500 italic" data-translate="true">Tidak ada hasil</div>`; if(typeof AutoTranslator !== 'undefined') AutoTranslator.processDOM(); return; }
        let htmlArray = [];
        filtered.forEach(i => {
        ...
        list.innerHTML = htmlArray.join(''); if(typeof AutoTranslator !== 'undefined') AutoTranslator.processDOM();
    }
    ...
    function compressImage(base64Str, maxWidth = 1000, quality = 0.6) {
        return new Promise((resolve, reject) => {
            ...
            img.onload = () => { try { const canvas = document.createElement('canvas'); let width = img.width; let height = img.height; if (width > maxWidth) { height *= maxWidth / width; width = maxWidth; } canvas.width = width; canvas.height = height; const ctx = canvas.getContext('2d'); ctx.drawImage(img, 0, 0, width, height); resolve(canvas.toDataURL('image/jpeg', quality)); } catch(e) { reject("Canvas error: " + e.message); } }; img.onerror = () => resolve(base64Str);
        });
    }

    // --- LIVE NOTIFICATION SCRIPT ---
    function toggleNotifDropdown() { document.getElementById('notif-dropdown').classList.toggle('hidden'); }
    document.addEventListener('click', function(e) { if(!e.target.closest('#notif-dropdown') && !e.target.closest('[onclick="toggleNotifDropdown()"]')) { const notifDd = document.getElementById('notif-dropdown'); if(notifDd && !notifDd.classList.contains('hidden')) notifDd.classList.add('hidden'); } });
    function goToApproval(targetFilterStr = '') { document.getElementById('notif-dropdown').classList.add('hidden'); let targetFilter = targetFilterStr || "{{ $targetFilter }}"; if(targetFilter !== '') { localStorage.setItem('auto_filter_gi', targetFilter); window.location.href = "{{ url('/gi') }}"; } }

    function updateNotifUI(count, target) {
        const btn = document.getElementById('notif-bell-btn'); const icon = document.getElementById('notif-bell-icon'); const badgeCont = document.getElementById('notif-badge-container'); const badgeCount = document.getElementById('notif-badge-count'); const ddHeaderCount = document.getElementById('notif-dd-count'); const ddBody = document.getElementById('notif-dd-body');
        if (!btn || !icon || !badgeCont) return;
        if (count > 0) {
            btn.className = "relative w-8 h-8 sm:w-10 sm:h-10 bg-white rounded-full flex items-center justify-center shadow-sm transition btn-animated text-red-500 hover:text-red-700"; icon.className = "far fa-bell animate-ring"; badgeCont.classList.remove('hidden'); badgeCount.innerText = count > 9 ? '9+' : count;
            if(ddHeaderCount) { ddHeaderCount.classList.remove('hidden'); ddHeaderCount.innerText = count + ' New'; }
            if(ddBody) { ddBody.innerHTML = `<div onclick="goToApproval('${target}')" class="p-4 border-b border-slate-50 hover:bg-red-50 cursor-pointer transition flex gap-3 group"><div class="w-8 h-8 rounded-full bg-red-100 text-red-500 flex items-center justify-center flex-shrink-0 group-hover:bg-red-600 group-hover:text-white transition"><i class="fas fa-exclamation-circle text-xs"></i></div><div><p class="text-xs font-black text-slate-800 mb-0.5" data-translate="true">Tindakan Diperlukan</p><p class="text-[10px] text-slate-500 leading-tight">Anda memiliki <span class="font-bold text-red-600">${count}</span> dokumen GIS yang menunggu aksi Anda.</p><p class="text-[9px] text-red-600 font-bold mt-1.5 opacity-0 group-hover:opacity-100 transition transform group-hover:translate-x-1" data-translate="true">Klik untuk memproses &rarr;</p></div></div>`; }
        } else {
            btn.className = "relative w-8 h-8 sm:w-10 sm:h-10 bg-white rounded-full flex items-center justify-center shadow-sm transition btn-animated text-slate-500 hover:text-red-600"; icon.className = "far fa-bell"; badgeCont.classList.add('hidden');
            if(ddHeaderCount) ddHeaderCount.classList.add('hidden');
            if(ddBody) { ddBody.innerHTML = `<div class="p-6 text-center text-slate-400"><i class="far fa-check-circle text-3xl mb-3 opacity-30"></i><p class="text-xs font-bold" data-translate="true">Semua selesai.</p><p class="text-[10px] mt-1" data-translate="true">Tidak ada tugas yang tertunda.</p></div>`; }
        }
        if(typeof AutoTranslator !== 'undefined') AutoTranslator.processDOM();
    }

    function checkLiveNotifications() {
        if(typeof currentUser === 'undefined') return;
        fetch('{{ url("api/gis.php") }}', { method: 'POST', body: JSON.stringify({ action: 'getRequests' }) }).then(r => r.json()).then(d => {
            if(d.code === 401) return; let reqs = Array.isArray(d) ? d : (d.data || []); let count = 0; let target = ''; const role = currentUser.role; const dept = currentUser.department; const isWh = ['Warehouse', 'Administrator'].includes(role) || (role === 'TeamLeader' && dept.toLowerCase() === 'warehouse');
            if (role === 'Administrator') { let c1 = reqs.filter(r => r.status === 'Pending Head').length; let c2 = reqs.filter(r => r.status === 'Pending Plant Head').length; let c3 = reqs.filter(r => r.status === 'Pending Warehouse').length; let c4 = reqs.filter(r => r.status === 'Pending No GI (ERP)').length; count = c1 + c2 + c3 + c4; if(c1 > 0 || c2 > 0) target = 'Pending Head'; else if(c3 > 0) target = 'Pending Warehouse'; else if(c4 > 0) target = 'Completed'; } else if (isWh) { let c3 = reqs.filter(r => r.status === 'Pending Warehouse').length; let c4 = reqs.filter(r => r.status === 'Pending No GI (ERP)').length; count = c3 + c4; if(c3 > 0) target = 'Pending Warehouse'; else if(c4 > 0) target = 'Completed'; } else if (role === 'PlantHead') { count = reqs.filter(r => r.status === 'Pending Plant Head').length; if(count > 0) target = 'Pending Head'; } else if (['SectionHead', 'TeamLeader'].includes(role)) { count = reqs.filter(r => r.status === 'Pending Head' && r.department === dept).length; if(count > 0) target = 'Pending Head'; } else { count = reqs.filter(r => r.status === 'Pending Receive' && r.username === currentUser.username).length; if(count > 0) target = 'Pending Receive'; }
            updateNotifUI(count, target);
        }).catch(e => console.log('Notif check skipped'));
    }

    document.addEventListener("DOMContentLoaded", () => {
        const autoFilter = localStorage.getItem('auto_filter_gi');
        if (autoFilter && typeof setGiFilter === 'function') { let checkDataInterval = setInterval(() => { if(typeof giData !== 'undefined' && giData.length > 0) { clearInterval(checkDataInterval); localStorage.removeItem('auto_filter_gi'); setGiFilter(autoFilter); } }, 300); setTimeout(() => { clearInterval(checkDataInterval); localStorage.removeItem('auto_filter_gi'); }, 5000); }
        checkLiveNotifications(); setInterval(checkLiveNotifications, 10000);
    });

    // --- MANAGE USERS SCRIPT ---
    function openManageUsers() { openModal('modal-users'); loadUsers(); }
    function loadUsers() { fetch('{{ url("api/users.php") }}', {method:'POST', body:JSON.stringify({action:'getAllUsers'})}).then(r=>r.json()).then(d => { if(d.code===401){logoutAction(); return;} allUsers = Array.isArray(d) ? d : (d.data || []); renderUsers(allUsers); }); }
    ...
    function resetUserForm() { document.getElementById('user-form').reset(); document.getElementById('u-user').disabled = false; document.getElementById('u-pass').required = true; document.getElementById('btn-del-user').classList.add('hidden'); document.getElementById('form-title').innerText = 'Create User'; document.querySelectorAll('.acc-chk').forEach(chk => { chk.checked = false; chk.disabled = false; }); document.getElementById('chk-gi-submit').checked = true; if(typeof AutoTranslator !== 'undefined') AutoTranslator.processDOM(); }

    function editUser(u) {
        const user = allUsers.find(x => x.username === u);
        document.getElementById('u-user').value = user.username; document.getElementById('u-user').disabled=true; document.getElementById('u-pass').value = ''; document.getElementById('u-pass').required=false; document.getElementById('u-name').value = user.fullname; document.getElementById('u-nik').value = user.nik || ''; document.getElementById('u-role').value = user.role; document.getElementById('u-dept').value = user.department; document.getElementById('u-phone').value = user.phone || ''; document.getElementById('btn-del-user').classList.remove('hidden'); document.getElementById('form-title').innerText = 'Edit User';
        let acc = []; if (typeof user.access_rights === 'string') { try { acc = JSON.parse(user.access_rights); } catch (e) { acc = []; } } else if (Array.isArray(user.access_rights)) { acc = user.access_rights; }
    ...
    function saveUser() {
        ...
        fetch('{{ url("api/users.php") }}', {method:'POST', body:JSON.stringify(p)}).then(r=>r.json()).then(res => { if(res.code === 401) { logoutAction(); return; } if(res.success){ resetUserForm(); loadUsers(); showCustomAlert("Success", "Berhasil simpan user."); } else showCustomAlert("Error", res.message); });
    }

    function deleteUser() { showCustomConfirm("Delete User", "Hapus user ini?", () => { fetch('{{ url("api/users.php") }}', {method:'POST', body:JSON.stringify({action:'deleteUser', username:document.getElementById('u-user').value})}).then(r=>r.json()).then(res => { if(res.success){ resetUserForm(); loadUsers(); } }); }); }

    // --- FITUR BULK UPLOAD & DOWNLOAD USER ---
    function downloadUserTemplate() {
        const rows = [
            ["Username", "Password", "Fullname", "NIK", "Department", "Role", "Phone", "Access Rights (Pisahkan dengan koma)"],
            ["johndoe", "123456", "John Doe", "12345", "IT", "User", "081234567890", "gi_submit, item_add"],
            ["janedoe", "123456", "Jane Doe", "54321", "Warehouse", "Warehouse", "081298765432", "gi_submit, gr_submit, item_add, item_edit, stock_edit, export_data, price_add, price_edit, item_delete, edit_gi_no, edit_gr_no"]
        ];
        const wb = XLSX.utils.book_new(); const ws = XLSX.utils.aoa_to_sheet(rows); XLSX.utils.book_append_sheet(wb, ws, "Template User"); XLSX.writeFile(wb, `Template_Import_User.xlsx`);
    }

    function exportUsers() {
        if (allUsers.length === 0) { showCustomAlert("Info", "Tidak ada data user untuk diekspor."); return; }
        const rows = [["Username", "Password", "Fullname", "NIK", "Department", "Role", "Phone", "Access Rights (Pisahkan dengan koma)"]];
        allUsers.forEach(u => {
            let accStr = "";
            try { let accArr = []; if(typeof u.access_rights === 'string') accArr = JSON.parse(u.access_rights); else if(Array.isArray(u.access_rights)) accArr = u.access_rights; accStr = accArr.join(", "); } catch(e) {}
            rows.push([u.username, "", u.fullname, u.nik || '', u.department, u.role, u.phone || '', accStr]);
        });
        const wb = XLSX.utils.book_new(); const ws = XLSX.utils.aoa_to_sheet(rows); XLSX.utils.book_append_sheet(wb, ws, "Data Users"); XLSX.writeFile(wb, `Data_Users_${new Date().getTime()}.xlsx`);
    }

    async function importUsers(event) {
        const file = event.target.files[0]; if (!file) return;
        const ext = file.name.split('.').pop().toLowerCase();
        if (ext !== 'xlsx' && ext !== 'xls' && ext !== 'csv') { showCustomAlert("Error", "Format file harus Excel (.xlsx/.xls) atau .csv"); event.target.value = ''; return; }

        document.getElementById('form-title').innerHTML = `<i class="fas fa-spinner fa-spin mr-2 text-emerald-500"></i> Memproses Import...`;

        const reader = new FileReader();
        reader.onload = async function(e) {
            try {
                const data = new Uint8Array(e.target.result); const workbook = XLSX.read(data, {type: 'array'}); const firstSheet = workbook.SheetNames[0];
                const rawRows = XLSX.utils.sheet_to_json(workbook.Sheets[firstSheet], {defval: ''});
                if (rawRows.length === 0) throw new Error("File kosong atau format salah.");

                let successCount = 0; let failCount = 0;
                for (let i = 0; i < rawRows.length; i++) {
                    const rawRow = rawRows[i]; const row = {};
                    for (let key in rawRow) { row[key.toString().trim().toLowerCase()] = rawRow[key]; }

                    if (!row.username || !row.fullname || !row.role || !row.department) { failCount++; continue; }

                    const targetUsername = row.username.toString().trim();
                    const isExisting = allUsers.some(u => u.username === targetUsername);

                    let acc = []; let roleStr = row.role.toString().trim();
                    if (roleStr === 'Administrator') {
                        acc = ['gi_submit', 'gr_submit', 'item_add', 'item_edit', 'stock_edit', 'export_data', 'price_add', 'price_edit', 'item_delete', 'edit_gi_no', 'edit_gr_no'];
                    } else {
                        let accKey = Object.keys(row).find(k => k.includes('access'));
                        if (accKey && row[accKey]) { acc = row[accKey].toString().split(',').map(s => s.trim()).filter(s => s !== ""); }
                        if (acc.length === 0 && roleStr === 'Warehouse') { acc = ['gi_submit', 'gr_submit', 'item_add', 'item_edit', 'stock_edit', 'export_data', 'price_add', 'price_edit', 'item_delete', 'edit_gi_no', 'edit_gr_no']; }
                    }

                    const payload = {
                        action: 'saveUser', isEdit: isExisting,
                        data: { username: targetUsername, password: row.password ? row.password.toString().trim() : '123456', fullname: row.fullname.toString().trim(), nik: row.nik ? row.nik.toString().trim() : '', role: roleStr, department: row.department.toString().trim(), phone: row.phone ? row.phone.toString().trim() : '', access_rights: JSON.stringify(acc) }
                    };

                    const r = await fetch('{{ url("api/users.php") }}', {method: 'POST', body: JSON.stringify(payload)});
                    const res = await r.json();
                    if(res.success) successCount++; else failCount++;
                }

                event.target.value = ''; loadUsers();
                showCustomAlert("Import Complete", `Success: ${successCount} users saved.\nFailed/Skipped: ${failCount} lines.`);
                document.getElementById('form-title').innerText = 'Create User';
            } catch (err) {
                event.target.value = ''; document.getElementById('form-title').innerText = 'Create User';
                showCustomAlert("Error", "Gagal membaca file Excel. Pastikan format sesuai template.");
            }
        };
